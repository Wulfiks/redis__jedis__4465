  ...
  public static <T> T aggregateDefault(T existing, T newReply) {
    ...
    if (existing instanceof Map && newReply instanceof Map) {
      ...
      try {
        ...
        Map<Object, Object> merged = new java.util.LinkedHashMap<>(existingMap.size() + newMap.size());
        merged.putAll(existingMap);
        merged.putAll(newMap);
        return (T) merged;
      }
    ...
    if (existing instanceof Set && newReply instanceof Set) {
      ...
      try {
        ...
        Set<Object> merged = new java.util.LinkedHashSet<>(existingSet.size() + newSet.size());
